...
# Nome eseguibile
exec_name="deterministic_OPENMP_cc"
...
for cpu in ${cpus[@]}; do
    ...
    echo "#PBS -l select=1:ncpus=${cpu}:mem=${ram}gb -l place=excl" >> "results_${exec_name}/script_${cpu}.sh"
    ...
    echo "./${exec_name}.out ${input}" >> "results_${exec_name}/script_${cpu}.sh"
    # Lancio lo script
